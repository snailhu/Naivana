...
$('#getinfo').click(function(){
	if($('.month-picker').val()!=''){
		...
		else if($('#position').find("option:selected").attr('original')=='sis'){
			var standardUrl='/Nirvana/backend/web/api/area/globalgoal';
			url='/Nirvana/backend/web/api/area/bigarea/goal';
		}
		else{
			return false;
		}
    	  		...
    	  		if(json.response=="success"){
	    	  		...
	    	  		for(i in info){
		                   ...
		                   '<td>'+nr+'</td>'+
		                    ...
		                    '<td>'+info[i].areaName+info[i].positionName+'</td>'+
    ...
    $('#save').click(function(){
    	var data={}
    	$('.nrval').each(function(){
    		data[$(this).parent().parent().attr('data-id')]=$(this).val()==""?0:$(this).val();
    	})
    	var info={};
    	info[area]=areaId;
    	info['year']=year;
