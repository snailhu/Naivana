package com.nirvana.controller.dto.web;

import com.nirvana.domain.backend.area.AgentArea;
import com.nirvana.pojo4json.BaseDTO;

public class WebCreateAgentAreaDTO extends BaseDTO<AgentArea> {

	private Integer id;

	private String code;

	private String name;

	public WebCreateAgentAreaDTO() {
	}

	public Integer getId() {
		return id;
	}

	public void setId(Integer id) {
		this.id = id;
	}

	public String getCode() {
		return code;
	}

	public void setCode(String code) {
		this.code = code;
	}

	public String getName() {
		return name;
	}

	public void setName(String name) {
		this.name = name;
	}

	@Override
	public BaseDTO<AgentArea> convert(AgentArea domain) {
		WebCreateAgentAreaDTO dto = new WebCreateAgentAreaDTO();
		dto.setId(domain.getId());
		dto.setCode(domain.getAreaCode());
		dto.setName(domain.getDesc());
		return dto;
	}
}
